...
<div class="container-fluid">
	<div class="row-fluid">
		<div class="row-fluid">
				<div align='left'><legend><?php echo ($date); ?>的弹窗信息</legend></div>
				<span style="float:left">
					<input type="text" placeholder="请选择日期" id="date" name="date" class='datepicker' value='<?php echo ($date_value); ?>' /> 
					<a class='btn'
						style="margin-top: 10px; margin-bottom: 10px" onclick="window.location.href='/Config/poplist/?date='+$('#date').val()" href="#"><i
						class="icon"></i>查看</a>
				</span>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pop): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($type[$pop['type']]); ?></td>
							<td><a href='<?php echo ($edit_link[$pop['type']]); echo ($pop['id']); ?>' ><?php echo ($pop["title"]); ?></a>
