	...
	<div class="row-fluid">
		<div class="row-fluid">
				<span style="float:left"><a class='btn btn-success'
					style="margin-top: 10px; margin-bottom: 10px" href="/ContentPop/add"><i
				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pop): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($pop["title"]); ?>
								<?php if($pop['disabled'] == 1): ?><b style='color:red;'>[停用]</b><?php endif; ?></td>
							<td><?php echo ($pop["app_id"]); ?></td>
							<td><?php echo ($pop["app_name"]); ?></td>
							<td><?php echo ($sub_type[$pop['sub_type']]); ?></td>
							...
							<td><a class="btn" href="/ContentPop/edit?id=<?php echo ($pop['id']); ?>&locale=<?php echo ($pop['locale']); ?>"><i
									class="icon  icon-edit"></i>编辑</a>&nbsp; 
									<a class="btn" href="/ContentPop/add?copy_from_id=<?php echo ($pop['id']); ?>&locale=<?php echo ($pop['locale']); ?>"><i
									class="icon  icon-share"></i>复制</a>&nbsp;
									<a class="btn" href="/ContentPop/destroy?id=<?php echo ($pop['id']); ?>"
