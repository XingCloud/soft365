				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							<td width=300><?php echo ($v["name"]); ?></td>
							<td><a class="btn" href="destroy?name=<?php echo ($v['name']); ?>"
								onclick="return confirm('确认要删除吗?')"><i	class="icon  icon-trash"></i>删除</a>
							</td>
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
						<tr>
							<td><input type="text" placeholder="请填写<?php echo ($name); ?>名称" id="name" name="name"  /> 
					</td>
							<td><input class='btn' style="margin-top: 10px; margin-bottom: 10px" 
